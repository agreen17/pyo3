...
fn setitem() {
    ...
    let c = Py::new_ref(py, SetItem { key: 0, val: 0 }).unwrap();
    py_run!(py, c, "c[1] = 2");
    assert_eq!(c.key, 1);
    assert_eq!(c.val, 2);
...
fn delitem() {
    ...
    let c = Py::new_ref(py, DelItem { key: 0 }).unwrap();
    py_run!(py, c, "del c[1]");
    assert_eq!(c.key, 1);
...
fn setdelitem() {
    ...
    let c = Py::new_ref(py, SetDelItem { val: None }).unwrap();
    py_run!(py, c, "c[1] = 2");
...
fn context_manager() {
    ...
    let c = Py::new_mut(py, ContextManager { exit_called: false }).unwrap();
